...
it('displays the given label message when one is supplied', () => {
    ...
    const { getByText } = render(<InputString component={ component } isLoading={ false } onChange={ jest.fn } />);
...
it('has a maximum number of characters when one is given', () => {
    ...
    const { container } = render(<InputString component={ component } isLoading={ false } onChange={ jest.fn } />);
...
it('has a width set when one is given', () => {
    ...
    const { container } = render(<InputString component={ component } isLoading={ false } onChange={ jest.fn } />);
    expect(container.querySelector('#id-1234')).toHaveAttribute('width', '1234');
});
